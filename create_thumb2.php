...
$strQueryPerPage="select main_image_url,id,user_defined_sku_id,main_image_url_backup from flagging_searplusitems where user_defined_sku_id='50007' limit $pagefrom,10";
$strResultPerPage=mysql_query($strQueryPerPage);
while($strResultPerPageRow=mysql_fetch_array($strResultPerPage))
{
	echo "<br> == >".$main_image_url=$strResultPerPageRow['main_image_url_backup'];
	echo " == >".$FileName=trim($strResultPerPageRow['user_defined_sku_id']).".jpg";

	$ImgPath=preg_replace("[^A-Za-z0-9]","_",$main_image_url);
...
function runurl()
{
	window.location.href='create_thumb2.php?page=<?=$page;?>';
}
</script>
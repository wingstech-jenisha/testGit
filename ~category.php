...
if($Catid!='')
{
	$CattQry="SELECT jet_node_id FROM flagging_category WHERE parent_id='".$Catid."'";
	...
	if(mysql_affected_rows()>0)
	{
		while($CattRow=mysql_fetch_array($CattRes))
		{
			...
			$CattQry2="SELECT jet_node_id FROM flagging_category WHERE parent_id='".$CatSID."'";
			...
			if(mysql_affected_rows()>0)
			{
				while($CattRow2=mysql_fetch_array($CattRes2))
				{
					...
					$CattQry3="SELECT jet_node_id FROM flagging_category WHERE parent_id='".$CatPID."'";
...
<head>
<meta charset="utf-8">
<base href="<?=$SITE_URL;?>/"  /><title><?=$MetaPageTitle;?></title>
...
<style>body {background: #ffffff;}.category-pro-img-detail h3{height:40px;overflow:hidden}.category-detail-right h2 a {color: #000;text-transform: uppercase;font-size:20px;padding: 20px 0px;}</style>
<link rel="stylesheet" type="text/css" href="css/responsive.css<?=$CSSLOAD;?>" />
<style>@media only screen and (max-width:767px){.category-pro-img-detail h3{height:auto;overflow:hidden;}}
</style>
<? include("headermore.php");?>
<script>
function gogtoto(id){qtyyX=1; AddtoCartAjaxDtl(id,id,'',qtyyX);}
function AddtoCartAjaxDtl(AddtoCartAjax_ID,itemid,price,quantity)
{
	...
	http7333.onreadystatechange=function(){if(http7333.readyState == 4) {window.location.href='items-in-your-cart';return false; }}
	http7333.send(null);
}
		...
		<? if($pppttot>0){?>
			  ...
			  ?>
				  <div class="category-pro-box">
					<div class="category-pro-img"> <? if($image_url!=''){?><a href="<?=$ItmUrl;?>"><img src="<?=stripslashes($image_url);?>" alt="<?=stripslashes($SelMovieObj->user_defined_sku_id);?>, <?=stripslashes($SelMovieObj->product_title);?>, <?=$siteALTTXT;?>" title="<?=stripslashes($SelMovieObj->user_defined_sku_id);?>, <?=stripslashes($SelMovieObj->product_title);?>, <?=$siteALTTXT;?>" /></a><? }else{ ?><img src="noimg.jpg"  style="max-height:170px;" alt="<?=stripslashes($SelMovieObj->user_defined_sku_id);?>, <?=stripslashes($SelMovieObj->product_title);?>, <?=$siteALTTXT;?>" title="<?=stripslashes($SelMovieObj->user_defined_sku_id);?>, <?=stripslashes($SelMovieObj->product_title);?>, <?=$siteALTTXT;?>"><? } ?> </div>
					<div class="category-pro-img-detail">
					  <h3<?php /*?> style="height:63px;overflow:hidden"<?php */?>><a href="<?=$ItmUrl;?>"><?=stripslashes($tittl);?></a></h3>
					  <p<?php /*?> style="height:25px;"<?php */?>>SKU# <?=$SelMovieObj->user_defined_sku_id;?></p>
					  <span class="price">$<?=stripslashes($SelMovieObj->item_price);?></span>
					  <? if($inventory>0){?><div class="add-btn"> <a href="<?=$ItmUrl;?>">Add To Cart</a> </div><? }else{ ?><div class="add-btn"> <a href="#" onClick="return false;">Out of Stock</a> </div><? } ?>
					</div>
				  </div>
          		<? } ?>
          	
			<div class="clear"></div>
			<? if ($pppttot>$ShowPage) { ?><div class="pagging"><? if($PageNo>1){ $PrevPageNo = $PageNo -1;?><a href="<?=$PaginUrrl;?>Page=<?=$PrevPageNo;?>" class="butt">&laquo;<span class="mobpg"> Previous</span></a></li><? } ?>
			<? if($PageNo<=10)
			{
			...
			<a href="<?=$PaginUrrl;?>Page=<?=$NextPageNo;?>" class="butt"><span class="mobpg">Next </span>&raquo;</a>
			<? } ?>
		  </div>
		  <? } ?>
          ...
          <div class="clear">&nbsp;</div>
		  <? }else{ ?>
		  <div class="clear">&nbsp;</div><div class="clear" align="center" style="min-height:300px;"><strong>No matching records found.</strong></div><div class="clear">&nbsp;</div>
	    <? } ?>
        </div>
      </div>
      <div class="clear"></div>
...
{
	var http2SF = false;if(navigator.appName == "Microsoft Internet Explorer") {http2SF = new ActiveXObject("Microsoft.XMLHTTP");} else {http2SF = new XMLHttpRequest();}
	http2SF.abort();http2SF.open("GET", "ajax_validation.php?Type=AddToWatchListSEAERCH&vehicleid="+vehicleid, true);
...
</script><? include("googleanalytic.php");?><? include("dbclose.php");?>
</body>
</html>
